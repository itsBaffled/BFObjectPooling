...
- Various Customization options so each pool can be totally different from the last
	...
	- Runtime changing of pool type (Assuming its of same class or a child, see how the BP handle does it by just using UObject. )
...
```cpp
 ...
 TBFObjectPoolPtr<AMyFoo, ESP::ThreadSafe> MyPool;   
